...
def load_nvdcve():
    ...
    for i in years:
        ...
        with open(fname, 'r') as infile:
            data = json.load(infile)["CVE_Items"]
        nvdcve.extend(data)
    return nvdcve

def parse_nvdcve(nvdcve):
    out = []
    print('Parsing data')
    impacts = 0
    noImpacts = 0
...
def chunk_yaml_list(input, name, size=20000):
...
if __name__ == '__main__':
    ...
    if os.path.isfile(nvdcvefile):
        ...
        nvdcve = parse_nvdcve(load_nvdcve())
        with open(nvdcvefile, 'w') as outfile:
            json.dump(nvdcve, outfile, indent=2)
    # now we have nvdcve, one way or another
    # need to break into 8192 token chunks
    #chunkList = chunk_yaml_list(nvdcve[0:20], size=2000)
    chunkList = chunk_yaml_list(nvdcve, 'nvdcve', size=20000) # default size of 20000
    print("Number of chunks: %d" % len(chunkList))
    # that works. now try the model
    nvdcve_embedding = embed_text_list(chunkList)
    with open('nvdcve_embedding.json', 'w') as outfile:
